    ...
    >
      ${pathStrings.join("")}
		...
		try {
			...
			// Transpiles JSX into JS with Babel for CommonJS
			const resultCJS = await babel.transformAsync(svgComponent, {
				...BABEL_SETTINGS,
				presets: [...BABEL_SETTINGS.presets, ["@babel/preset-env"]],
				presets: [...BABEL_SETTINGS.presets, ['@babel/preset-env']]
			});

			fs.outputFile(path.join(componentPath, "index.js"), resultCJS.code);

			// Generate ESM version
			const resultESM = await babel.transformAsync(svgComponent, {
				...BABEL_SETTINGS,
				presets: [
					...BABEL_SETTINGS.presets,
					['@babel/preset-env', { modules: false }]
				]
			});

			fs.outputFile(path.join(componentPath, 'index.esm.js'), resultESM.code);
...
async function generateIconComponent() {
	...
	const iconComponentTemplateESM = await ejs.renderFile(
		path.join(FOLDER.TEMPLATES, "IconComponentESM.jsx"),
		{
